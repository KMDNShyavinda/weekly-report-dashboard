...
import { getAdminReviewReports, updateReportReview } from '../../api/reportApi';
...
function MetricCard({ label, value, icon, accent }) {
  return (
    <div style={{ borderRadius: '18px', padding: '1rem', background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(148,163,184,0.18)' }}>
...
export default function ReportReviews() {
  ...
  useEffect(() => {
    const loadReports = async () => {
      try {
        const { data } = await getAdminReviewReports();
        setReports(data || []);
      } catch (error) {
        console.error(error);
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '24px 20px 40px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.2rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', flexWrap: 'wrap' }}>
            <button className="animated-button" onClick={toggle} style={{ border: `1px solid ${theme.border}`, background: theme.surface, color: theme.text, borderRadius: '999px', padding: '0.65rem 0.95rem', cursor: 'pointer' }}>
              {dark ? '☀️ Light' : '🌙 Dark'}
            </button>
            <Link className="animated-link" to="/dashboard" style={{ border: `1px solid ${theme.border}`, background: theme.surface, color: theme.text, borderRadius: '999px', padding: '0.65rem 0.95rem' }}>Dashboard</Link>
            <button className="animated-button" onClick={logout} style={{ border: 'none', background: theme.gradient, color: '#fff', borderRadius: '999px', padding: '0.65rem 0.95rem', cursor: 'pointer' }}>Logout</button>
          </div>
        </div>
        ...
        {!id ? (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem', marginBottom: '1.2rem' }}>
              <MetricCard label="Total Reports" value={reports.length} icon={<FileText size={18} />} accent={theme.primary} />
              <MetricCard label="Pending Review" value={reports.filter((item) => (item.reviewStatus || 'pending') === 'pending').length} icon={<Clock3 size={18} />} accent="#f59e0b" />
              <MetricCard label="Under Review" value={reports.filter((item) => item.reviewStatus === 'under_review').length} icon={<AlertTriangle size={18} />} accent="#3b82f6" />
              <MetricCard label="Approved / Rejected" value={reports.filter((item) => ['approved', 'rejected'].includes(item.reviewStatus)).length} icon={<CheckCircle2 size={18} />} accent="#16a34a" />
            </div>
            ...
            <div style={{ borderRadius: '24px', padding: '1.2rem', background: theme.surface, border: `1px solid ${theme.border}`, boxShadow: theme.shadow }}>
              <div style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', marginBottom: '1rem' }}>
                ...
                <label style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', fontSize: '0.9rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.55rem', borderRadius: '12px', padding: '0.7rem 0.8rem', border: `1px solid ${theme.border}`, background: dark ? '#111827' : '#f8fafc' }}>
                    <Filter size={16} />
                    <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ border: 'none', outline: 'none', width: '100%', background: 'transparent', color: theme.text }}>
                      ...
                      {pagedReports.map((item) => (
                        <tr key={item._id} style={{ borderTop: `1px solid ${theme.border}` }}>
                          ...
                          <td style={{ padding: '0.85rem 0.6rem' }}>
                            <button onClick={() => navigate(`/admin/report-reviews/${item._id}`)} style={{ border: 'none', background: theme.gradient, color: '#fff', borderRadius: '10px', padding: '0.5rem 0.75rem', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}>
                              <Eye size={14} /> View
                            </button>
                          </td>
                        </tr>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', flexWrap: 'wrap', gap: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} style={{ border: `1px solid ${theme.border}`, background: theme.surface, color: theme.text, padding: '0.55rem 0.8rem', borderRadius: '999px', cursor: page === 1 ? 'not-allowed' : 'pointer', opacity: page === 1 ? 0.7 : 1 }}>Previous</button>
                  <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} style={{ border: `1px solid ${theme.border}`, background: theme.surface, color: theme.text, padding: '0.55rem 0.8rem', borderRadius: '999px', cursor: page === totalPages ? 'not-allowed' : 'pointer', opacity: page === totalPages ? 0.7 : 1 }}>Next</button>
                </div>
              </div>
            </div>
          </>
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
              <div>
                <Link to="/admin/report-reviews" style={{ color: theme.primary, fontWeight: 700 }}>← Back to review queue</Link>
            ...
            <div style={{ position: 'sticky', top: '12px', zIndex: 10, display: 'flex', gap: '0.7rem', flexWrap: 'wrap', borderRadius: '16px', padding: '0.9rem', background: theme.surface, border: `1px solid ${theme.border}`, boxShadow: theme.shadow }}>
              <button onClick={() => handleReviewAction('under_review', 'under review')} disabled={submitting} style={{ border: `1px solid ${theme.border}`, background: dark ? '#111827' : '#f8fafc', color: theme.text, padding: '0.6rem 0.9rem', borderRadius: '999px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}><MessageSquareText size={15} /> Start Review</button>
              <button onClick={handleDownload} style={{ border: `1px solid ${theme.border}`, background: dark ? '#111827' : '#f8fafc', color: theme.text, padding: '0.6rem 0.9rem', borderRadius: '999px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}><Download size={15} /> Download</button>
              <button onClick={handlePrint} style={{ border: `1px solid ${theme.border}`, background: dark ? '#111827' : '#f8fafc', color: theme.text, padding: '0.6rem 0.9rem', borderRadius: '999px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}><Printer size={15} /> Print</button>
            </div>
              ...
              <div style={{ display: 'grid', gap: '1rem' }}>
                <div style={{ borderRadius: '24px', padding: '1rem', background: theme.surface, border: `1px solid ${theme.border}`, boxShadow: theme.shadow }}>
                  ...
                  <label style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', marginBottom: '0.7rem' }}>
                    ...
                    <textarea value={reviewNotes} onChange={(e) => setReviewNotes(e.target.value)} rows={4} style={{ borderRadius: '12px', border: `1px solid ${theme.border}`, padding: '0.7rem', background: dark ? '#111827' : '#fff' }} />
                  </label>
                  <label style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', marginBottom: '0.7rem' }}>
                    <span style={{ color: theme.muted, fontSize: '0.9rem' }}>Internal Notes</span>
                    <textarea value={internalNotes} onChange={(e) => setInternalNotes(e.target.value)} rows={4} style={{ borderRadius: '12px', border: `1px solid ${theme.border}`, padding: '0.7rem', background: dark ? '#111827' : '#fff' }} />
                  </label>
                  <label style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                    <span style={{ color: theme.muted, fontSize: '0.9rem' }}>Feedback for Client</span>
                    <textarea value={feedbackForClient} onChange={(e) => setFeedbackForClient(e.target.value)} rows={4} style={{ borderRadius: '12px', border: `1px solid ${theme.border}`, padding: '0.7rem', background: dark ? '#111827' : '#fff' }} />
                  </label>
                  <button onClick={handleSaveNotes} disabled={submitting} style={{ marginTop: '0.8rem', border: 'none', background: theme.gradient, color: '#fff', padding: '0.65rem 0.9rem', borderRadius: '999px', cursor: 'pointer' }}>Save notes</button>
                </div>
                  ...
                  <div style={{ display: 'grid', gap: '0.8rem' }}>
                    {(selectedReport?.timeline || []).length ? selectedReport.timeline.map((item, index) => (
                      <div key={index} style={{ borderRadius: '14px', padding: '0.8rem', background: dark ? '#111827' : '#f8fafc' }}>
                        ...
                        <div style={{ color: theme.muted, marginTop: '0.25rem', fontSize: '0.8rem' }}>By {item.actor || user?.name || 'Admin'}</div>
                      </div>
                    )) : (
