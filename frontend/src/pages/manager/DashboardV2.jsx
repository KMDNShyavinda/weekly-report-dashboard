...
import { getThemeTokens, getTheme } from '../../styles/theme';
...
export default function DashboardV2() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      value: summary?.rejectedReports ?? 0,
      icon: '❌',
      color: '#ef4444',
      accent: '#fee2e2',
      trend: 2
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '24px 20px 40px' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', flexWrap: 'wrap' }}>
            ...
            <Link className="animated-link" to="/report-reviews" style={{ border: `1px solid ${theme.border}`, background: theme.surface, color: theme.text, borderRadius: '999px', padding: '0.65rem 0.95rem', boxShadow: theme.shadow }}>
              Report Reviews
            </Link>
        ...
        <div style={{ marginTop: '1.5rem', display: 'grid', gap: '1rem', gridTemplateColumns: '1fr', maxWidth: '100%' }}>
          <section style={{ borderRadius: '24px', background: theme.surface, border: `1px solid ${theme.border}`, boxShadow: theme.shadow, padding: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800 }}>Manager AI Assistant</h2>
                <p style={{ marginTop: '0.5rem', color: theme.muted, maxWidth: '640px' }}>
                  Ask the assistant about team activity, blockers, and workload balance. Responses are grounded in the latest reports.
                </p>
              </div>
              <div>
                <Link to="/assistant" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', borderRadius: '999px', padding: '0.75rem 1rem', background: theme.primary, color: '#fff', textDecoration: 'none', fontWeight: 700, boxShadow: theme.shadow }}>
                  Open Assistant
                  <ArrowRight size={16} />
                </Link>
              </div>
